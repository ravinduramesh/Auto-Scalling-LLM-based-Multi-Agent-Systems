...
scatterColors = []
for label in agentCountsAndBinaryWeight.keys():
    if label.split("\n")[1].startswith("autogen"):
        scatterColors.append('pink')
    elif label.split("\n")[1].startswith("DRTAG"):
        scatterColors.append('limegreen')
    else:
        scatterColors.append('yellow')


i = 0
for key in agentCountsAndBinaryWeight.keys():
    plt.scatter(agentCountsAndBinaryWeight[key][0], agentCountsAndBinaryWeight[key][1], color=scatterColors[i])
    i += 1

plt.title("Agent Count vs Binary Weight for New Knowledge")
plt.xlabel("Number of Agents")
plt.ylabel("Binary Weight for New Knowledge")
plt.legend(loc="upper right", title="LLM-based MAS approach", labels=["Autogen", "DRTAG", "IAAG"])
plt.grid()
plt.tight_layout()
